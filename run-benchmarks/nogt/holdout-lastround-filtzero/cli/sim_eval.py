...
deepfitness_dir = os.path.join(config.PRJ_DIR, 'deepfitness')

layout_df = pd.read_csv(os.path.join(config.DATA_DIR, 'layout.csv'))
...
method_to_fitness_col = {
    'sfit-mul': 'Simple inferred fitness',
    'sfit-dirmul': 'Simple inferred fitness',
    'sfit-latent': 'Inferred fitness',
    'acides': 'ACIDES inferred fitness',
    'enrich2': 'Enrich2 inferred fitness',
}
method_to_inpfn = {
    'sfit-mul': 'simplefitness.csv',
    'sfit-dirmul': 'simplefitness.csv',
    'sfit-latent': 'simple_latent.csv',
    'acides': 'merged.csv',
    'enrich2': 'merged.csv',
}
# only run simeval-latent on `latent` methods:
latent_methods = ['acides', 'sfit-latent']
...
def get_simeval_latent_command(inference_path: InferencePath) -> str:
    ...
    if method == 'acides':
        cmd = ' '.join([
            'python -m deepfitness.scripts.research.sim_eval_acides',
            f'--csv {os.path.join(inference_path.path, method_to_inpfn[method])}',
            f'--genotype_col Genotype',
            f'--round_before {round_before}',
            f'--round_after {round_after}',
        ...
        cmd = ' '.join([
            ...
            f'--genotype_col Genotype',
            f'--fitness_col \"{method_to_fitness_col[method]}\"',
            f'--log_abundance_col \"__Log abundance\" '
...
def main(args: dict):
    ...
    out_csv = os.path.join(OUT_DIR, 'holdout-lastround-filtzero.csv')
